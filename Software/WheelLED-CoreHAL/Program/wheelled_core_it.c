...
{
  if(__HAL_TIM_GET_FLAG(&WLED_TIM_HandleStruct, TIM_FLAG_UPDATE) != RESET) {
    if(__HAL_TIM_GET_IT_SOURCE(&WLED_TIM_HandleStruct, TIM_IT_UPDATE) != RESET) {
      __HAL_TIM_CLEAR_IT(&WLED_TIM_HandleStruct, TIM_IT_UPDATE);
      WLED_UpdateEven(&WLED_TIM_HandleStruct);
    }
  }
}
