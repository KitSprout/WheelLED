...
int16_t MoveAve_WMA( int16_t newData, int16_t *moveAveFIFO, uint16_t windowLens )
{
  ...
  for(i = 0; i < windowLens - 1; i++)               // 陣列移動
    moveAveFIFO[i] = moveAveFIFO[i+1];
  moveAveFIFO[windowLens - 1] = newData;            // 加入新數據
  for(i = 0; i < windowLens; i++)                   // 求和 & 加權
    sumData += moveAveFIFO[i]*(i+1);
  sampleSum = (windowLens * (windowLens + 1)) / 2;  // 計算加權除數
  aveData = (int16_t)(sumData / sampleSum);         // 計算平均值
